  ...
  useEffect(() => {
    ...
    axios.get("https://dzartisansapp.onrender.com/services/get", { withCredentials: true })
      .then(response => {
        ...
        setartisanTypes(services.map(service => service.nom))
        console.log(artisanTypes)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        console.log("Devis sent successfully:", response.data);
        alert("Votre demande de devis a été envoyée avec succès.");
        navigate("/demandedeviss"); // Navigate to another page if needed
      } else {
        console.error("Error sending devis:", response.data);
